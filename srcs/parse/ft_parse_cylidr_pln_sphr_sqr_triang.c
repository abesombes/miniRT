...
int	ft_parse_cylinder(char *line, t_scene *sc, int err_code, int *obj_id)
{
	...
	new_obj->cylinder.radius = ft_atoif(&line, &err_code);	
	if (new_obj->cylinder.radius < 0 || !err_code)
		return (-12);
	ft_move_to_next_data(&line);
	new_obj->cylinder.height = ft_atoif(&line, &err_code);
	if (new_obj->cylinder.height < 0 || !err_code)
		return (-15);
...
int	ft_parse_square(char *line, t_scene *sc, int *obj_id)
{
	...
	if (err_code)
		return (err_code == -6 ? -10: err_code);
	ft_move_to_next_data(&line);
	new_obj->square.l = ft_atoif(&line, &err_code);
	if (new_obj->square.l < 0 || !err_code)
		return (-14);
	ft_get_rgb(&line, new_obj, &err_code);
	ft_display_vec(&new_obj->rgb);
	return (err_code ? err_code : 0);
